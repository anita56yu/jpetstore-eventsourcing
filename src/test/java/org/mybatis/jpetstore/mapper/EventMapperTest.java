...
import org.mybatis.jpetstore.event.OrderPlaced;
...
class EventMapperTest {
  ...
  void insertEvent() {
    // given
    IEvent event = new OrderPlaced("cookie", 1);
    event.setEventId(1);
    // when
    mapper.insertEvent(event);

    // then
    Map<String, Object> record = jdbcTemplate.queryForMap("SELECT * FROM events WHERE eventid = ?", 1);
    assertThat(record).hasSize(25).containsEntry("EVENTID", event.getEventId())
        .containsEntry("ITEMID", event.getItemId()).containsEntry("AMOUNT", event.getAmount());
  }
